...
public class UsersController extends BaseController {
	...
	public Map getRoleDisplay() throws Exception {
		...
		return ImmutableMap.of("type", "success", "data", sb.toString());
	...
	public Map getUsersRoleList(@RequestBody UsersFormBean userid)
		...
		if (StringUtils.isBlank(userid.getCreateUserName())) {
			return ImmutableMap.of("type", "success", "username", "",
					"password", "", "email", "", "roles", this
							.populateUserRolsesCreate(userrolesSetting
									.getAllRoles()));
		} else {
			...
			if (user != null) {
				return ImmutableMap.of(
						...
						user.getEmail(),
						"roles",
						this.populateUserRolsesModify(
								userrolesSetting.getAllRoles(), user));
			} else {
				return ImmutableMap.of("type", "success", "username",
						"username1", "password", "", "email", "email1",
						"roles", this.populateUserRolsesCreate(userrolesSetting
								.getAllRoles()));
			}
		}
	}
	...
	private String populateUserRolsesModify(Map<String, String> userroles,
			UsersBean user) {
		StringBuffer sb = new StringBuffer();
