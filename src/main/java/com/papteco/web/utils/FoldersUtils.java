	...
	public void createProjectFolders(String projectPath,
		...
		for (FolderBean folder : folderList) {
			...
			if (!sf.exists()) {
				...
				// log.info("(execable, readable, writeable) - ("+sf.canExecute()+", "+sf.canRead()+", "+sf.canWrite()+") - "+
				// projectPath+"/"+folder.getFolderName());
			} else {
				// log.info("(execable, readable, writeable) - ("+sf.canExecute()+", "+sf.canRead()+", "+sf.canWrite()+") - "+
				// projectPath+"/"+folder.getFolderName()+" [existing already]");
			}
		}
